...
SaaS-платформа для мониторинга транзакций в финтех-компаниях, необанках и платёжных сервисах. Выявляет подозрительные операции в соответствии с AML-требованиями (AMLD5/6, BSA/FinCEN, 115-ФЗ).

---

## Для чего нужен этот проект?

Это учебный проект — **MVP (минимально жизнеспособный продукт)** для демонстрации подхода к мониторингу отмывания денег.

**Важно:** Перед реальным использованием система должна пройти:
1. Юридическую проверку на соответствие локальному законодательству
2. Аудит информационной безопасности
3. Валидацию модели детекции независимым комплаенс-экспертом
...
**Как работает:**
1. **Frontend** (React/Next.js) показывает интерфейс для compliance-офицеров
2. **Backend** (FastAPI) обрабатывает запросы, управляет данными
3. **Celery Worker** — фоновый процесс, который:
   - Скачивает транзакции из блокчейнов (BTC, ETH, USDT, XMR)
   - Запускает детекторы AML-правил
   - Создаёт алерты о подозрительных операциях
4. **PostgreSQL** — основная база данных
5. **Redis** — очередь задач для Celery и кэш
...
### 4. UI для compliance-офицера
- Просмотр алертов с фильтрацией
- Изменение статуса (new → in_review / escalated / closed)
- История изменений (audit log с hash chain)
- Экспорт SAR-отчётов
